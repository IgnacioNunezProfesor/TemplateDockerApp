...
if [ ! -d "${DB_DATADIR}/mysql" ]; then   # Comprueba si el directorio de datos no existe
...
mariadbd-safe --user=${DB_USER} --datadir=${DB_DATADIR} &  # Inicia MariaDB en background
...
/usr/bin/mariadb -u root <<EOF  # Ejecuta comandos SQL como root
...
if [ -d "/entrypointsql" ]; then  # Comprueba si existe el directorio
    for f in /entrypointsql/*.sql; do  # Recorre todos los .sql
        if [ -f "$f" ]; then  # Verifica que el archivo existe
            ...
            /usr/bin/mariadb -u root -p"${DB_ROOT_PASS}" < "$f"  # Ejecuta el script SQL
        fi
    done
fi
